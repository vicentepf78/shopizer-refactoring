...
@Component
public class ReferenceServiceClientRestTemplateImpl implements ReferenceServiceClient {

	private final RestTemplate referenceRestTemplate;
	private final String baseUrl;

	public ReferenceServiceClientRestTemplateImpl(
			@Qualifier("referenceRestTemplate") RestTemplate referenceRestTemplate,
	...
	private <T> List<T> exchangeList(String url, ParameterizedTypeReference<List<T>> type) {
		try {
			...
			throw new ReferenceUnavailableException("Reference service call failed: " + url, e);
		}
	}
}
