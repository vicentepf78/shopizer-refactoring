...
class CheckoutApplicationServicePlaceOrderTest {
	...
	@BeforeEach
	void setUp() {
		lenient().when(outboxProperties.isEnabled()).thenReturn(false);
	...
	void whenOutboxEnabledPayPalOmitsCommandTransaction() throws Exception {
		...
		commandTransaction.setTransactionDetails(details("PAYER-1", "TOKEN-1"));
		Order builtOrder = new Order();
	...
	void apiFlowDelegatesToOrderServiceWithPreBuiltOrder() throws Exception {
		...
		when(merchantStoreService.getByCode("DEFAULT")).thenReturn(store);
		when(languageService.getByCode("en")).thenReturn(language);
		when(orderService.processOrder(eq(preBuilt), eq(customer), any(), eq(summary), eq(payment), isNull(), eq(store)))
		...
		verify(orderService).processOrder(preBuilt, customer, command.getShoppingCartItems(), summary, payment, null,
				store);
	}
