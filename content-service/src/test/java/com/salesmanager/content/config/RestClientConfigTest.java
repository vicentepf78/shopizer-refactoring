...
import com.salesmanager.content.web.CorrelationIdFilter;
...
class RestClientConfigTest {
	...
	void correlationInterceptorCopiesInboundHeader() {
		RestTemplate restTemplate = new RestClientConfig()
				.referenceRestTemplate(new RestTemplateBuilder(), 1000);
		MockRestServiceServer server = MockRestServiceServer.createServer(restTemplate);

		MockHttpServletRequest servletRequest = new MockHttpServletRequest();
		servletRequest.addHeader(CorrelationIdFilter.HEADER, "corr-outbound-1");
		RequestContextHolder.setRequestAttributes(new ServletRequestAttributes(servletRequest));

		server.expect(requestTo("http://example/health"))
				.andExpect(header(CorrelationIdFilter.HEADER, "corr-outbound-1"))
