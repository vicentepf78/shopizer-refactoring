...
class ReferenceServiceClientTest {
	...
	@Test
	void blankInputs_returnNullWithoutHttpCall() {
		assertThat(client.getCountryByCode("", "en")).isNull();
		assertThat(client.getCountryByCode(null, "en")).isNull();
		assertThat(client.getZoneByCode("", "QC", "en")).isNull();
		assertThat(client.getZoneByCode("CA", "", "en")).isNull();
		assertThat(client.getLanguageByCode("")).isNull();
		assertThat(client.getLanguageByCode(null)).isNull();
	}

	@Test
	void getCountryByCode_withoutLang_omitsLangQueryParam() {
		server.expect(requestTo("http://reference-test:8081/api/v1/country"))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withSuccess(
						"[{\"id\":1,\"code\":\"CA\",\"name\":\"Canada\",\"supported\":true,\"zones\":[]}]",
						MediaType.APPLICATION_JSON));

		assertThat(client.getCountryByCode("CA", null).getCode()).isEqualTo("CA");
		server.verify();
	}

	@Test
	void getZoneByCode_withoutLang_omitsLangQueryParam() {
		server.expect(requestTo("http://reference-test:8081/api/v1/zones?code=CA"))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withSuccess(
						"[{\"id\":10,\"code\":\"QC\",\"name\":\"Quebec\",\"countryCode\":\"CA\"}]",
						MediaType.APPLICATION_JSON));

		assertThat(client.getZoneByCode("CA", "QC", null).getCode()).isEqualTo("QC");
		server.verify();
	}

	@Test
	void getLanguageByCode_unknown_returnsNull() {
		server.expect(requestTo("http://reference-test:8081/api/v1/languages"))
				.andRespond(withSuccess("[]", MediaType.APPLICATION_JSON));

		assertThat(client.getLanguageByCode("xx")).isNull();
		server.verify();
	}
}
