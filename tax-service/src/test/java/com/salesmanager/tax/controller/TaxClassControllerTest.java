...
class TaxClassControllerTest {
	...
	void setUp() throws Exception {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(new TaxClassController(taxFacade))
	...
	@Test
	void create_returnsEntityId() throws Exception {
		...
		mockMvc.perform(post("/api/v1/private/tax/class")
						...
						.content("{\"code\":\"T1\",\"name\":\"One\"}"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(5));
	}

	@Test
	void unique_returnsExistsFlag() throws Exception {
		when(taxFacade.existsTaxClass(eq("T1"), any(), any())).thenReturn(false);

		mockMvc.perform(get("/api/v1/private/tax/class/unique").param("code", "T1"))
				.andExpect(status().isOk())
