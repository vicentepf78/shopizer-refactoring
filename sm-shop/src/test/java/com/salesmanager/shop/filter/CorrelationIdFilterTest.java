...
class CorrelationIdFilterTest {
	...
	void propagatesExistingHeader() throws Exception {
		CorrelationIdFilter filter = new CorrelationIdFilter();
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(CorrelationIdFilter.HEADER, "corr-shop-1");
		MockHttpServletResponse response = new MockHttpServletResponse();
		AtomicReference<String> seen = new AtomicReference<>();
		...
		assertThat(seen.get()).isEqualTo("corr-shop-1");
		assertThat(response.getHeader(CorrelationIdFilter.HEADER)).isEqualTo("corr-shop-1");
	}
}
